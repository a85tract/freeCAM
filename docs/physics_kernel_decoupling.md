...
| `cldfrc_fice` | DeepConvection (hooked; called inside the compiled `zm_conv_evap`) | function contract; standalone image | bindable, gates owed | a hoisted copy of `zm_conv_evap` was not bit-for-bit (7343258, failure record kept); now reached through a hook that redirects `zm_conv.o`'s call, machine code untouched | open: hook gates, capture and replay |
| `fluxbelowinv` | ShallowConvection (hooked; private, called three times per column inside the compiled `compute_uwshcu`) | function contract; standalone image through its own symbol | bindable, gates owed | its weakened definition hands the three call sites to the hook | open: hook gates, capture and replay |

...
def test_session_ui_lists_every_supported_pi_cam_physics_interface(
    ...
    assert len(session.physics) == 302
    assert len(set(session.physics.names)) == 302
    ...
    assert sum(process.runnable for process in session.physics) == 40
    assert not hasattr(session.physics, "by_phase")
    ...
    assert "302 flat physics interfaces" in html

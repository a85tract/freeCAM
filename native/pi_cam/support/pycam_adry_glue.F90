! tphysac (physpkg.F90:1533-1536) hoisted for the
...
module pycam_adry_glue
  ...
  use pycam_adry_driver, only: driver_bind
  implicit none
  private
...
contains
  ...
  subroutine glue_enter(lchnk_in)
    ...
    if (status /= 0_c_int .or. ndims /= 1_c_int) call endrun('pycam_adry_runner: carry surfric refused')
    call c_f_pointer(address, surfric, (/ int(extents(1)) /))
    status = pycam_ac_carry_v1(int(lchnk_in, c_int), 2_c_int, address, ndims, extents)
    if (status /= 0_c_int .or. ndims /= 1_c_int) call endrun('pycam_adry_runner: carry obklen refused')
  ...
  subroutine glue_bind_driver()
    ! physpkg.F90:1534-1534: the call's actuals bound to the unit's dummies
    call driver_bind( state, pbuf, obklen, surfric, cam_in, ztodt, cam_out, ptend )
  end subroutine glue_bind_driver

  subroutine glue_piece_1()
    ! physpkg.F90:1533-1533, verbatim
  ...
  subroutine glue_piece_2()
    ! physpkg.F90:1535-1536, verbatim

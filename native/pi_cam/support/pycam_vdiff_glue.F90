! tphysac (physpkg.F90:1481-1513) hoisted for the
...
module pycam_vdiff_glue
  ...
  use pycam_vdiff_driver, only: driver_bind
  implicit none
  private
  public :: glue_piece_1, glue_piece_2, glue_piece_3, glue_piece_4, glue_enter, glue_bind_driver, glue_configure
  ...
  real(r8), pointer, save, public :: surfric(:) => null()
...
contains
  ...
  subroutine glue_enter(lchnk_in)
    ...
    call glue_view(comsrf_sgh30(:, lchnk_in), address)
    call c_f_pointer(address, sgh30, (/ pcols /))
    cam_out => host_cam_out(lchnk_in)
    state => host_state(lchnk_in)
    tend => host_tend(lchnk_in)
    pbuf => pbuf_get_chunk(host_pbuf2d, lchnk_in)
    status = pycam_ac_carry_v1(int(lchnk_in, c_int), 1_c_int, address, ndims, extents)
    if (status /= 0_c_int .or. ndims /= 1_c_int) call endrun('pycam_vdiff_runner: carry surfric refused')
    call c_f_pointer(address, surfric, (/ int(extents(1)) /))
    status = pycam_ac_carry_v1(int(lchnk_in, c_int), 2_c_int, address, ndims, extents)
    if (status /= 0_c_int .or. ndims /= 1_c_int) call endrun('pycam_vdiff_runner: carry obklen refused')
    call c_f_pointer(address, obklen, (/ int(extents(1)) /))
    lchnk = state%lchnk
    ncol = state%ncol
    nstep = get_nstep()
  ...
  subroutine glue_bind_driver()
    ! physpkg.F90:1491-1494: the call's actuals bound to the unit's dummies
    call driver_bind(ztodt ,state ,cam_in%wsx, cam_in%wsy, cam_in%shf ,cam_in%cflx ,surfric ,obklen ,ptend ,ast , cam_in%ocnfrac , cam_in%landfrac , sgh30 ,pbuf )
  ...
  subroutine glue_piece_1()
    ! physpkg.F90:1483-1486, verbatim
  ...
  subroutine glue_piece_2()
    ! physpkg.F90:1490-1490, verbatim
  ...
  subroutine glue_piece_3()
    ! physpkg.F90:1499-1504, verbatim
  ...
  subroutine glue_piece_4()
    ! physpkg.F90:1511-1513, verbatim

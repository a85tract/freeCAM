...
module pycam_hooks
  ...
  logical, parameter :: has_model(nhooks) = (/ .false., .false., .true. /)
...
contains
  ...
  subroutine model_instratus_condensate(lchnk, ncol, k, p_in, t0_in, qv0_in, ql0_in, qi0_in, ni0_in, a_dc_in, ql_dc_in, qi_dc_in, a_sc_in, ql_sc_in, qi_sc_in, landfrac, snowh, rhmini_in, rhminl_in, rhminl_adj_land_in, rhminh_in, t_out, qv_out, ql_out, qi_out, al_st_out, ai_st_out, ql_st_out, qi_st_out)
    ...
    n = min(int(ncol), 16)
    t_out(1:n) = o_t_out(1:n)
    n = min(int(ncol), 16)
    qv_out(1:n) = o_qv_out(1:n)
    n = min(int(ncol), 16)
    ql_out(1:n) = o_ql_out(1:n)
    n = min(int(ncol), 16)
    qi_out(1:n) = o_qi_out(1:n)
    n = min(int(ncol), 16)
    al_st_out(1:n) = o_al_st_out(1:n)
    n = min(int(ncol), 16)
    ai_st_out(1:n) = o_ai_st_out(1:n)
    n = min(int(ncol), 16)
    ql_st_out(1:n) = o_ql_st_out(1:n)
    n = min(int(ncol), 16)
    qi_st_out(1:n) = o_qi_st_out(1:n)

#!/usr/bin/env python3
"""Compare a Python-driven macrophysics trace against the oracle's capture.

``Macrophysics.tend`` writes, when ``FREECAM_MACRO_TRACE`` is set, one JSON
line per chunk per step holding the live-lane hash of every mmacro_pcond
argument as it entered the kernel and of every returned value as it left.
The capture bundle (``tools/convert_pi_cam_function_capture.py``) holds the
same arguments the oracle passed, for every chunk of every rank at steps 1
and 2.  This joins the two on ``(mpi_rank, lchnk, nstep)`` and reports, for
each record, the first argument whose bytes differ -- before the call (the
...
answered differently, or the answer was copied out wrongly).
    ...
    tools/compare_pi_cam_macro_trace.py \\
...
def load_trace(directory: Path) -> dict[tuple[int, int, int], dict]:
    records: dict[tuple[int, int, int], dict] = {}
    for path in sorted(directory.glob("macro_trace.rank-*.jsonl")):
...
def compare(bundle_path: Path, trace_dir: Path) -> dict:
...
def main() -> int:
    ...
    report = compare(arguments.bundle, arguments.trace)
